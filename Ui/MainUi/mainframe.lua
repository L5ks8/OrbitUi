-- Moon UI Mainframe (preserves Orbit API structure, matches Moon UI visual hierarchy)
return function(mainfunctions, components)
    ...
    function Library:CreateWindow(config)
        ...
        if not targetParent then
            error("Moon UI: Could not locate a valid GUI parent container.")
        end

        if targetParent:FindFirstChild("Moon") then
            targetParent["Moon"]:Destroy()
        end
        ...
        -- Loading overlay (matching Moon G2L["3"])
        ...
        New("UIScale", {Scale = 1}, G2L["logo"])

        -- Empty frame (matching Moon G2L["7"])
        G2L["empty"] = New("Frame", {
            Size = UDim2.new(1, 0, 1, 0),
            Position = UDim2.new(0.5, 0, 0.5, 0),
            AnchorPoint = Vector2.new(0.5, 0.5),
        ...
        -- UIPageLayout inside main frame (matching Moon G2L["8"])
        ...
        -- Content container (matching Moon G2L["9"])
        ...
        CollectionService:AddTag(G2L["navigation"], "MoonScreenButton")

        -- Navigation UIPadding (matching Moon G2L["d"])
        New("UIPadding", {
        ...
        -- User frame (matching Moon G2L["e"])
        G2L["user"] = New("Frame", {
        ...
        CollectionService:AddTag(G2L["user_frame"], "MoonScreenButton")
        ...
        CollectionService:AddTag(G2L["avatar"], "ThemesOnlyIncludeBg")
        ...
        -- Fixed buttons (matching Moon G2L["1e"])
        ...
        -- UIListLayout in navigation (matching Moon G2L["32"])
        G2L["nav_list"] = New("UIListLayout", {
            FillDirection = Enum.FillDirection.Vertical,
            SortOrder = Enum.SortOrder.LayoutOrder,
            Padding = UDim.new(0, 0)
        }, G2L["navigation"])

        -- Directory ScrollingFrame (matching Moon G2L["33"])
        ...
        -- Primary tab container (matching Moon G2L["36"])
        ...
        -- Screen CanvasGroup (matching Moon G2L["85"])
        ...
        -- LeafletControls (matching Moon G2L["58b"])
        ...
        CollectionService:AddTag(G2L["b"], "MoonResizeHandle")
