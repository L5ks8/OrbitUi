...
-- Define repository path for remote loading
local baseUrl = "https://raw.githubusercontent.com/L5ks8/UiLibary/main/Ui/"

-- Load Main Functions dynamically
local mainfunktions
if getgenv().GoonHubDev then
    -- Developer mode: Load local file
    local success, content = pcall(readfile, "Ui/mainfunktions.lua")
    if success then
        local func, err = loadstring(content)
        if func then 
            mainfunktions = func() 
        else
            error("GoonHub Dev: Error loading mainfunktions: " .. tostring(err))
        end
    else
        error("GoonHub Dev: Could not read local mainfunktions.lua")
    end
else
    -- Production mode: Load from GitHub repository
    local success, content = pcall(game.HttpGet, game, baseUrl .. "mainfunktions.lua")
    if success then
        local func, err = loadstring(content)
        if func then 
            mainfunktions = func() 
        else
            error("GoonHub: Error loading mainfunktions: " .. tostring(err))
        end
    else
        error("GoonHub: Failed to fetch mainfunktions.lua from GitHub")
    end
end

-- Ensure mainfunktions loaded successfully
if not mainfunktions then
    error("GoonHub: Critical error - mainfunktions.lua is nil.")
end

-- Expose themes to Library
